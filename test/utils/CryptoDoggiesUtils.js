module.exports = function(CryptoDoggies, accounts) {
  ...
  function checkDoggyCreation(name) {
    it("createToken should create a random doggy named " + name, function(
      done
    ) {
      CryptoDoggies.deployed()
        .then(async function(instance) {
          await instance
            .createToken(name, { from: accounts[0] })
            .then(function(result) {
              assert.include(
                result.logs[0].event,
                "TokenCreated",
                "TokenCreated event was not triggered"
              );
  ...
  function checkDoggyPurchase(tokenId, price) {
    it("purchase should allow us to buy doggy tokenId: " + tokenId, function(
      done
    ) {
      CryptoDoggies.deployed()
        .then(async function(instance) {
  ...
  function checkWithdrawal(withdrawAmount) {
    it("withdrawBalance should withdraw " + withdrawAmount + " eth", function(
      done
    ) {
      CryptoDoggies.deployed()
        .then(function(instance) {
          instance
            .withdrawBalance(accounts[0], web3.toWei(withdrawAmount, "ether"), {
              from: accounts[0]
            })
            .then(function(result) {
              var contractBalance = web3.fromWei(
